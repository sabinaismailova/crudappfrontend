...
import { fetchCampusNameThunk } from "../campus/campus.action";
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_STUDENT_KEY}${student.id}`,
        student
      );
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await axios.post(
        process.env.REACT_APP_STUDENT_KEY,
        newStudent
      );
...
export const enrollStudent = (studentId, campusId ) => ({
  type: StudentActionType.ENROLL_STUDENT,
  payload: { studentId, campusId },
});

export const enrollStudentThunk = (studentId, campusId ) => {
  return async (dispatch) => {
    try {
